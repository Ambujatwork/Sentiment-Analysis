...
def get_mood_based_on_review(ai_review):
    """Suggest a mood based on AI-generated review sentiment."""
    sentiment, _ = analyze_sentiment(ai_review)
    if "Positive" in sentiment:
        return "Excited, Happy, or Inspired 🎉"
    elif "Neutral" in sentiment:
        return "Relaxed or Open-Minded ☕"
    else:
        return "Looking for a Thought-Provoking or Dark Story 🌑"


### 🚀 **FIXED IMAGE TEXT EXTRACTION USING GEMINI AI**
def extract_text_from_image(image):
    """Extract text from an image using Google Gemini AI (Updated to Gemini 1.5, with required 'role' field)."""
    try:
        ...
        data = {
            "contents": [
                {"role": "user", "parts": [{"text": "Extract and return any readable text from this image."}]},  # ✅ Required role
                {"role": "user", "parts": [{"inline_data": {"mime_type": "image/png", "data": image_base64}}]}
            ]
        }
        ...
        if response.status_code == 200:
            ...
            if "candidates" in result and "content" in result["candidates"][0]:
                ...
                return "❌ AI response format incorrect."
        else:
            return f"❌ AI API error: {response.status_code} - {response.text}"
    except Exception as e:
        return f"❌ AI text extraction failed: {str(e)}"
### 🚀 AI RESPONSE HANDLER ###
